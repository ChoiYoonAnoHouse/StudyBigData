...
def getCoffeeBeanStoreInfo(result):
    ...
    for i in range(1, 381+1):
        wd.get('https://www.coffeebeankorea.com/store/store.asp')
        time.sleep(0.5)

        try:
            wd.execute_script(f"storePop2('{i}')")
            time.sleep(0.5) # 팝업표시 후에 크롤링이 안되서 브라우저가 닫히는 것을 방지하기 위함.
            html = wd.page_source
...
def main():
    ...
    # csv 저장, 경로는 ./로 지정해서 저장하면 StudyBigData 스키마에 저장됨(파이썬과 주피터 노트북의 경로가 다름)
    coffeebean_df.to_csv('./coffeebean_shop_info.csv',index=True, encoding='utf-8')
    # 밑에로 경로 지정해서 저장하면, day03 폴더에 저장됨
