...
class Worker(QThread):
    ...
    # 대신 통신을 통해서 UI스레드가 그림을 그릴 수 있도록 통신을 수행한다.
    valChangeSignal = pyqtSignal(int)
    
    def __init__(self, parent):
        super().__init__(parent)
        self.parent = parent
        self.working = True # 클래스 내부변수 working을 만들어 지정
        
    def run(self):
        while self.working:
            for i in range(0, 10000000): # 응답없음이 발생!
                print(f'출력: {i}')
...
class qTemplate(QWidget):
    ...
    def addControls(self) -> None:
        ...
        self.worker.valChangeSignal.connect(self.updateProgress) # 스레드에서 받은 시그널은 UpdateProgress 함수에서 처리해준다.
        
        
    @pyqtSlot(int)
    def updateProgress(self, val): # val이 Woker스레드에서 전달받은 반복값
