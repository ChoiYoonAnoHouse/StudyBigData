...
class qTemplate(QWidget):
    start = 1 # api 호출할 때 시작하는 데이터의 시작 번호
    max_display = 100 # 한페이지에 나올 데이터 수를 지정하는 것
    saveResult = [] # 저장할 때 담을 데이터가 들어가는 자리(딕셔너리 리스트) -> DataFrame
    ...
    def strip_tag(self, title): # html의 태그를 없애주는 함수
        ret = title.replace('&lt;', '<')
        ret = ret.replace('&gt;', '>')
        ret = ret.replace('&quot;', '"')
        ret = ret.replace('&apos;', "'")
    ...
    def getNaverSearch(self, keyword, search, start, display):
        ...
        # 네이버 인증 추가
        req.add_header('X-Naver-Client-Id', 'GXhnLGtf3N9C0wOjBonG')
        req.add_header('X-Naver-Client-Secret', 'LVhZLkHOGv')
